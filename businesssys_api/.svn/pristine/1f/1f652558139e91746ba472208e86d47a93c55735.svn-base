...
class Tools {
     ...
     * @author zhaoxiang <zhaoxiang051405@gmail.com>
     */
    public static function userMd5($str, $auth_key = '',$ismd5=true) {
        if($str === '') return '';
        if (!$auth_key) {
            $auth_key = config('apiBusiness.AUTH_KEY');
        }
        $ismd5 && $str = md5($str);
        return md5(sha1($str) . $auth_key);
    }
    ...
    public static function isAdministrator($uid = '') {
        if (!empty($uid)) {
            $adminConf = config('apiBusiness.USER_ADMINISTRATOR');
