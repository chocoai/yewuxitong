...
class User extends Base {
    ...
    public function getUserinfo() {
        ...
        if ($id) {
            $data = (new SystemUser())->where('id', $id)->field('id,deptid,username,superid,superman,name,num,name,password,companyid,mobile,email,ranking,gender,remark')->find()->toArray();
            $deptarr = Tools::buildArrFromObj((new SystemDept())->getFullpath($data['deptid']));
            $data['deptpath'] = array_column($deptarr, 'id');
            $superdept = (new SystemDept())->getUpdept($data['deptid']);
            $data['surpath'] = ['name' => $data['superman'], 'deptname' => $superdept['name']];
            $groupid = SystemAuthGroupAccess::where('uid', $id)->value('groupid');
            $data['groupid'] = str2arr($groupid);
    ...
    public function getUsers() {
        ...
        $uidArr = SystemAuthGroupAccess::where('', 'exp', 'FIND_IN_SET(' . $gid . ',groupid)')->column('uid');
        $listObj = SystemUser::alias('u')
                        ->join('__SYSTEM_USER_DATA__ su', 'su.uid = u.id', 'left')
                        ->whereIn('u.id', $uidArr)
                        ->order('u.create_time DESC')
                        ->field('u.*,su.logintimes,su.lastloginip,su.lastlogintime,su.uid')
                        ->group('u.id')
                        ->paginate($limit, false, ['page' => $start])->toArray();
        ;
     ...
     * @apiParam {string} deptpath   部门全路径
     * @apiParam {string} superid   上级主管id
     * @apiParam {string} companyid   上级主管id
     * @apiParam {string} superman   上级主管名字
     * @apiParam {string} remark   备注
     *
     */
    ...
    /**
     * @api {post} admin/User/newUserByDeptId 部门获取用户[admin/User/newUserByDeptId]
     * @apiVersion 1.0.0
     * @apiName newUserByDeptId
     * @apiGroup User
     * @apiSampleRequest admin/User/newUserByDeptId
     * @apiParam {int}  deptId   部门id
     * @apiParam {int}  name   用户名称
     */
    public function newUserByDeptId() {
        $deptId = input('post.deptId', '', 'int');
        $name = input('post.name', '');
        //获取部门下所有用户
        $condition['status'] = 1;
        $condition['is_deleted'] = 0;
        $name !== '' && $condition['name'] = ['like', '%' . $name . '%'];
        $deptId !== '' && $condition['deptpath'] = ['like', '%[' . $deptId . ']%'];
        $result = SystemUser::where($condition)->field('name,id,deptid,deptname,superid')->select();
        return $this->buildSuccess($result);
    }

    /**
     * @api {post} admin/User/getUserAuthList 授权获取用户[admin/User/getUserAuthList]
     * @apiVersion 1.0.0
     * @apiName getUserAuthList
     * @apiGroup User
     * @apiSampleRequest admin/User/getUserAuthList
     */
    public function getUserAuthList() {
        ...
        $group_user = SystemAuthGroupAccess::where('', 'exp', 'FIND_IN_SET(' . $group . ',groupid)')->column('uid');
        $listInfo = SystemUser::where(['status' => 1, 'state' => ['in', [0, 1, 2]], 'id' => ['not in', $group_user]])//state 0试用 1转正 2实习 3兼职 4临时工 5离职 6外协 7社会分配
                ->where('id', '<>', 1)
                ->field('id,num,name,ranking,position,deptname,deptallname,jobfamily')
                ->order(['deptid' => 'asc', 'id' => 'asc'])
                ->select();
    ...
    /**
     * @api {post} admin/User/getSyncUser 同步用户数据[admin/User/getSyncUser]
     * @apiVersion 1.0.0
     * @apiName getSyncUser
     * @apiGroup User
     * @apiSampleRequest admin/User/getSyncUser
     */
    public function getSyncUser() {
        $syncUser = new \app\common\controller\OASystem();
        $result = $syncUser->syncSystemUser();
        if ($result['code'] == 1) {
            return $this->buildSuccess('', '用户数据同步成功！');
        } else {
            return $this->buildFailed(0, '用户数据同步失败，请联系管理员！');
        }
    }

    /**
     * @api {post} admin/User/getSyncUser 同步部门数据[admin/User/getSyncUser]
     * @apiVersion 1.0.0
     * @apiName getSyncUser
     * @apiGroup User
     * @apiSampleRequest admin/User/getSyncUser
     */
    public function getSyncDept() {
        $syncUser = new \app\common\controller\OASystem();
        $result = $syncUser->syncDept();
        if ($result['code'] == 1) {
            return $this->buildSuccess('', '部门数据同步成功！');
        } else {
            return $this->buildFailed(0, '部门数据同步失败，请联系管理员！');
        }
    }
