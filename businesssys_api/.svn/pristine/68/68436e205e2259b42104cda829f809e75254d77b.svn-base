...
class Ransomer extends Base {
    ...
    public function index() {
        ...
        $search && $where['x.order_sn|y.estate_name|y.estate_owner'] = ['like', "%{$search}%"];
            ...
            if ($dispatch) {
                ...
                if ($is_foreclosure == 1) {
                    ...
                    foreach ($dispatch as $vals) {
                        ...
                        if (!$this->message->AddmessageRecord($dispatch[0]['ransome_id'], 2, 1, $id, $orderSn, 202, '派单消息', '收到一条新的派单，订单号为' . $orderSn . '，请点击查看', 1, 1, 0, 0, '', 'PC正常派单', 'order_ransom_dispatch')) {
                            Db::rollback();
            ...
            if ($type != 'DQJK') {
                ...
                if ($estateInfo) {
                    foreach ($estateInfo as $key => $val) {
                        ...
                        $estateInfo[$key]['house_type_str'] = isset($dictonaryTypeArr['ORDER_HOUSE_TYPE'][$val['house_type']]) ? $dictonaryTypeArr['ORDER_HOUSE_TYPE'][$val['house_type']] : '';
                    }
                }
                ...
                if ($customerInfo) {
                    foreach ($customerInfo as $keys => $vals) {
                        ...
                        $customerInfo[$keys]['is_guarantee_str'] = $vals['is_guarantee'] == '1' ? '是' : '否';
    ...
    private function inProcess($id, $order_sn) {
        $workflow = new Workflow();
        $type = Order::where(['order_sn' => $order_sn])->value('type');
        $flow_id = $workflow->getFlowId($type.'_FINANCIAL');
