...
class OrderOthers extends Base
{
     ...
     * @apiParam {string} stage_code    审批状态
     * @apiParam {int} search_text     关键词
    ...
    public function discountApplyList(Request $request)
    {
        ...
        $pageSize = $request->get('limit', config('apiBusiness.ADMIN_LIST_DEFAULT'));
        $keywords = $request->get('search_text', '');
        $start_time = $request->get('start_time');
        $end_time = $request->get('end_time');
        $stage = $request->get('stage_code');
        $where = [];
        $start_time_stamp = strtotime($start_time);
        $end_time_stamp = strtotime($end_time);
        if ($start_time && $end_time) {
            if ($start_time_stamp > $end_time_stamp) {
                $where['or.create_time'] = ['between', [$end_time, $start_time + 86400 - 1]];
            } else if ($start_time_stamp < $end_time_stamp) {
                $where['or.create_time'] = ['between', [$start_time, $end_time + 86400 - 1]];
            } else {
                $where['or.create_time'] = ['between', [$start_time, $end_time + 86400 - 1]];
            }
        } else {
            $start_time && $where['or.create_time'] = ['egt', $start_time_stamp];
            $end_time && $where['or.create_time'] = ['elt', $end_time_stamp + 86400 - 1];
        }
        if ($create_uid) {
            if ($subordinates == 1) {
        ...
        $keywords && $where['o.order_sn|e.estate_name|o.finance_sn|or.process_sn'] = ['like', "%{$keywords}%"];
        $where['o.status'] = 1;
     ...
     * @apiSuccess {string} advanceMoney.plus_rate    加收费率
     */
    //@author: bordon
    public function discountOrderDetail(Request $request)
    {
        ...
        if ($this->checkOrderType($result->type)) {
            $advanceMoney = OrderAdvanceMoney::where('order_sn', $result->order_sn)->where('status', 1)
                ->field('id,advance_money,advance_day,advance_rate,advance_fee,plus_rate')->select();
        }
        return $this->buildSuccess([
            'orderInfo' => $result,
        ...
        try {
            ...
            if ($isXj) {
                foreach ($data['otherDiscount'] as $discount) {
                    ...
                    $new_money = $advance->advance_money * $advance->advance_day * $discount['new_rate'] / 100;
                    if (!is_null($advance->plus_rate)) {
                        $new_money += $advance->advance_money * $advance->plus_rate / 100;
                    }
                    if ($new_money < 3000) {
                        $new_money = 3000;
                    }
                    $arr[] = [
                        ...
                        'new_money' => sprintf('%.2f', $new_money), //现金现垫资费
                        'order_advance_money_id' => $advance->id
                    ];
                }
            } else {
//                预收担保费低于3000时应按3000收取
                $new_money = $orderTee->money * $data['guarantee_rate'] / 100;
                if ($new_money < 3000) {
                    $new_money = 3000;
                }
                $arr[] = [
                    ...
                    'new_money' => sprintf('%.2f', $new_money)  // 额度现担保费
                ];
            }
    ...
    public function editDiscount(Request $request)
    {
        ...
        try {
            ...
            if ($orderOther->category == 1) {
                ...
                $otherDiscount->new_money = sprintf('%.2f', $new_money);
            }
            // 现金
            if ($orderOther->category == 2) {
                foreach ($otherDiscount as &$v) {
                    ...
                    $new_money = $advance->advance_money * $advance->advance_day * $cur_dicount['new_rate'] / 100;
                    if (!is_null($advance->plus_rate)) {
                        $new_money += $advance->advance_money * $advance->plus_rate / 100;
                    }
                    if ($new_money < 3000) {
                    ...
                    $v->new_money = sprintf('%.2f', $new_money);
                    $v->save();
                }
            }
 ...
 */
    ...
    /**
     * @api {post} admin/OrderOthers/discountDetails  折扣订单基本信息(详情页面)[admin/OrderOthers/discountDetails]
     * @apiVersion 1.0.0
     ...
     * @apiParam {string} order_sn    订单编号
     * @apiParam {string} other_id    他业务表主键id
     * @apiParam {string} proc_id    申请列表详情不需要传该参数，审批列表需要传
     *
     * @apiSuccess {number}  is_approval       是否可以审核 1可以审核0不可以审核
     ...
     * @apiSuccess {string}  advance_money      垫资金额
     * @apiSuccess {string}  advance_day      垫资天数
    ...
    public function discountDetails(Request $request)
    {
        ...
        if (isset($data['proc_id']) && $data['proc_id']) {
            $proc = WorkflowProc::where('id', $data['proc_id'])->where('status', 0)->find();
        }
        $baseInfo = $this->getOrderBaseInfo($data['order_sn']);
        $discount_info = OrderOther::field('id,category,process_sn,reason')->where('order_sn', $data['order_sn'])->with('otherDiscounts')->find($data['other_id']);
        if (!empty($discount_info->other_discounts) && $discount_info->category == 2) {
            $discount_info_other = $discount_info->other_discounts;
            foreach ($discount_info_other as &$discount) {
                $advance = OrderAdvanceMoney::field('advance_money,advance_day,plus_rate')->find($discount->order_advance_money_id);
                $discount->advance_money = $advance->advance_money;
                $discount->advance_day = $advance->advance_day;
                $discount->plus_rate = $advance->plus_rate;
            }
        }
        ...
        return $this->buildSuccess([
            'is_approval' => isset($proc) && $proc ? 1 : 0,
            'base_info' => $baseInfo,
    ...
    public function discountApprovalList(Request $request)
    {
        ...
        $keywords && $map['o.order_sn|y.estate_name|o.finance_sn|x.process_sn'] = ['like', "%{$keywords}%"];
        $map['o.status'] = 1;
    ...
    private function updateOrderInfo(OrderOther $orderOther)
    {
        ...
        if ($orderOther->category == 1) {
            ...
            $orderTee->guarantee_fee = $otherDiscount->new_money;
        }
        ...
        if ($orderOther->category == 2) {
            ...
            foreach ($otherDiscount as $v) {
                ...
                $advanceMoney->advance_fee = $v->new_money;
