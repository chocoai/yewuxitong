...
class Check extends Base {
         ...
         *
         *@apiParam {int}  is_people   1 领取人 2 使用人
         ...
         * @apiSuccess {int} is_administrator  是否是管理员 0不是管理员 1是管理员
         *
         */

        public function showCheckList(){
            $isPeople = input('is_people')?:1;
            $ownerId = input('owner')?:0;
            $suborXs = input('subor')?:0;
            $timeType = input('time_type')?:1;
            ...
            $map = [];
            //用户判断
            if($this->userInfo['deptid'] == 17 || $this->userInfo['deptid'] == 18){//管理员
                if(!empty($ownerId) && $isPeople == 1) $map['owner']=['in', $ownerId];
                if(!empty($ownerId) && $isPeople == 2) $map['user']=['in', $ownerId];
            }else{
                if ($ownerId) {
                    if ($suborXs == 1) {
                        $userStr = SystemUser::getOrderPowerStr($ownerId);
                    } else {
                        $userStr = $ownerId;
                    }
                    if($isPeople == 1) $map['owner']=['in', $userStr];
                    if($isPeople == 2) $map['user']=['in', $userStr];
                }else{
                    $map['owner']=['in', $userId];
                }
            }
            ...
            if ($timeType == 1) {  //入库时间
                ...
                if($startTime && $endTime){
                    if($startTime > $endTime){
                        $startTime = $startTime + 86400;
                        $map['owner_time'] = array(array('egt', $endTime), array('elt', $startTime));
                    }else{
                        $endTime = $endTime + 86400;
                        $map['owner_time'] = array(array('egt', $startTime), array('elt', $endTime));
                    }
                }elseif($startTime){
                    $map['owner_time'] = ['egt',$startTime];
                }elseif($endTime){
                    $endTime = $endTime + 86400;
                    $map['owner_time'] = ['elt',$endTime];
                }
            }elseif ($timeType == 3){ //使用时间
                if($startTime && $endTime){
                    if($startTime > $endTime){
                        $startTime = $startTime + 86400;
                        $map['use_time'] = array(array('egt', $endTime), array('elt', $startTime));
                    }else{
                        $endTime = $endTime + 86400;
            ...
            try{
                ...
                if($this->userInfo['deptid'] == 17 || $this->userInfo['deptid'] == 18){
                    $resInfo['is_administrator'] = 1;
                }else{
                    $resInfo['is_administrator'] = 0;
                }
        ...
        public function transferDetermine()
        {
            ...
            if(empty($checkId)) return $this->buildFailed(ReturnCode::EMPTY_PARAMS, '支票id不能为空!');
            if(empty($informaTion)) return $this->buildFailed(ReturnCode::EMPTY_PARAMS, '处理意见不能为空!');
            //查询出支票的状态进行验证
