...
class CreditInquiryAttachment extends Model {
    ...
    public function filterCreditpic($id, $type, $newids) {
        ...
        if (!empty($delids))
            foreach ($delids as $key => $value) {
                if (!$this->updateCreditpic($id, $value, $type, ['status' => -1])) {
                    return FALSE;
                }
            }
        $attach_pic = [];
     ...
     * @return bool
     * @throws \think\db\exception\DataNotFoundException
     * @param $data 与数据库字段对应的数组集合  $id 征信id $aid图片id $typeAUTH授权证附件  APPROVAL审核附件  CREDIT征信报告'
     * @author zhongjiaqi 4.21
     */
    public function updateCreditpic($id, $aid, $type, $data) {
        $where = [
            'credit_inquiry_id' => $id,
            'type' => $type,
            'attachment_id' => $aid
    ...
    public function getUrl($id, $type) {
        ...
        if ($res) {
            foreach ($res as $key => $value) {
                ...
                $data[] = $this->Attachment->where('id', $value)->field('name,url,id,thum1')->find();
            }
            foreach ($data as $key => $value) {
                $data[$key]['url'] = config('uploadFile.url') . $value['url'];
                $data[$key]['thumb_url'] = config('uploadFile.url') . "/" . $value['thum1'];
            }
        }
        return $data;
    }
