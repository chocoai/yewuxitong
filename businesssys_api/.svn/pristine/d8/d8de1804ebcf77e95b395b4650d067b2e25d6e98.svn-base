...
class CreditInquiry extends Model {
     ...
     * @throws \think\db\exception\DataNotFoundException
     * @param $id 征信id 
     * @author zhongjiaqi 7.13（业务助理、理财经理只能看自己，审查助理、审查经理、管理员、审查部门经理无限制，业务部门经理看本部门）
     */
    public function getDataByuserinfo($uid, $auth_group, $group, $where) {
        if (check_auth($auth_group['business_assistant'], $group)) {
            $where['create_uid'] = $uid;
        } elseif (check_auth($auth_group['investment_advisor'], $group)) {
            $where['financing_manager_id'] = $uid;
        } elseif (check_auth($auth_group['branch_manager'], $group)) {
            $where['dept_manager_id'] = $uid;
        }
        return $where;
    }
     ...
     * @return bool
     * @throws \think\db\exception\DataNotFoundException
     * @param $certdata 证件数据集  
     * @param $id 被筛选用户
     * @param $customer_id 被筛选用户
     * @param $remarkbase 备注
     * @author zhongjiaqi 4.21
     */
    public function filterSamecard($id, $customer_id, $certdata, $remarkbase, $financing_manager_id, $financing_dept_id) {
        $this->Customercert = new CustomerCert();
        $this->Customercert->filterCreditcert($customer_id, $certdata); //同步到客户信息
        ...
        $dept_manager_id = !empty($financing_manager_id) ? db('system_user')->where('id', $financing_manager_id)->value('superid') : NULL;
        if (count($certdata) > 0) {
            $newdata = json_encode(array_merge($oldarray, $certdata), JSON_UNESCAPED_UNICODE); // 重组新数据
            return $this->updateCredit($id, ['dept_manager_id'=>$dept_manager_id,'certother' => $newdata, 'remark_base' => $remarkbase, 'customer_id' => $customer_id, 'financing_manager_id' => $financing_manager_id, 'financing_dept_id' => $financing_dept_id]);
        } else {
            return $this->updateCredit($id, ['dept_manager_id'=>$dept_manager_id,'remark_base' => $remarkbase, 'customer_id' => $customer_id, 'financing_manager_id' => $financing_manager_id, 'financing_dept_id' => $financing_dept_id]);
        }
    }
