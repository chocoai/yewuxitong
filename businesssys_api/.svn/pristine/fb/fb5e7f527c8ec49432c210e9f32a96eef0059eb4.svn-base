...
class News extends Base {
     ...
     * @Param {int} $dept    用户所属部门
     * */

    public static function processList($user_id,$group,$dept){
        $map['o.delete_time'] = NULL;
        $map['o.status'] = 1;
        $map['o.stage'] = ['in','1002,1003,1004,1005,1006,1007,1008,1009,1010,1011'];
        $map['wp.status'] = 0;
        $map['wp.is_back'] = 0;
        $map['wp.is_deleted'] = 1;
        ...
        if(count($resInfo['data']) <= 7 && ($dept == 17 || $dept == 18)){
