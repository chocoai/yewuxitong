...
class Credit extends Base {
        ...
        if (!empty($creditList['data'])) {
            foreach ($creditList['data'] as $key => $value) {
                ...
                if ($value['is_auth_accessory'] == 0 && $value['is_auth_accessory_py'] == 0) {
                    $creditList['data'][$key]['is_auth_accessory_text'] = ''; // 是否已收授权
                } elseif ($value['is_auth_accessory'] == 1 && $value['is_auth_accessory_py'] == 1) {
                    $creditList['data'][$key]['is_auth_accessory_text'] = '人行,鹏元';
                } else {
                    $creditList['data'][$key]['is_auth_accessory_text'] = $value['is_auth_accessory_py'] == 1 ? '鹏元' : '人行';
                }
        ...
        if ($id) {
            ...
            if ($data) {
                ...
                if ($data['is_auth_accessory'] == 0 && $data['is_auth_accessory_py'] == 0) {
                    ...
                    $search_data['is_auth_accessory_text'] = '人行,鹏元';
                } else {
                    $search_data['is_auth_accessory_text'] = $data['is_auth_accessory_py'] == 1 ? '鹏元' : '人行';
                }
    ...
    public function exportCredit() {
        ...
        $checkstatus = $this->request->post('checkstatus', '');
        $credit_result = $this->request->post('credit_result', '');
        $time_type = $this->request->post('time_type', '');
        $time = $this->request->post('time_solt/a', '');
        $fid = $this->request->post('financing_manager_id', '');
        $where = [];
        if ($fid) {
            $where['financing_manager_id'] = $fid;
        }
        ...
        $keywords && $where['customer_name|certcode'] = ['like', "%{$keywords}%"];
        $where['status'] = 1;
        $field = "id,order_sn,customer_name,certcode,is_auth_accessory,checkstatus,apply_time,sub_bank_time,credit_input_time,credit_result,remark_inquiry,remark_trial,create_uid,financing_manager_id,is_auth_accessory_py";
        ...
        if (!empty($creditList)) {
            ...
            foreach ($creditList as $key => $value) {
                ...
                $creditData[$key]['certcode'] = "\t".$value['certcode']."\t"; //证件号码   \t解决数字过长转换成科学计算              
                ...
                //$creditData[$key]['is_auth_accessory_text'] = $value['is_auth_accessory'] == 1 || $value['is_auth_accessory_py'] == 1? '是' : '否'; // 是否已收授权
                $auth_accessory=$value['is_auth_accessory'] == 1 ? '人行' : '';
                $auth_accessory_py=$value['is_auth_accessory_py'] == 1 ? '鹏元' : '';
                //授权类型
                if($value['is_auth_accessory'] == 1 && $value['is_auth_accessory_py'] == 1){
                    $creditData[$key]['accessory_type'] = $auth_accessory.';'.$auth_accessory_py;    
                }elseif($value['is_auth_accessory'] == 1){
                    $creditData[$key]['accessory_type'] = $auth_accessory;
                }elseif($value['is_auth_accessory_py'] == 1){
                    $creditData[$key]['accessory_type'] = $auth_accessory_py;
                }else{
                    $creditData[$key]['accessory_type']='';
                }
        ...
        //return print_r($creditData);
        try {
            $spreadsheet = new Spreadsheet();
            $resInfo = $creditData;
            $head = ['0' => '序号', '1' => '客户姓名', '2' => '证件号码', '3' => '所属部门', '4' => '理财经理',
                '5' => '业务单号', '6' => '查询状态', '7' => '申请时间', '8' => '申请人',
                '9' => '提交人行时间', '10' => '征信录入时间', '11' => '授权类型', '12' => '征信结果', '13' => '（征信查询信息）备注说明',
                '14' => '（征信审核信息）备注说明'];
            array_unshift($resInfo, $head);
            //$fileName = iconv("UTF-8", "GB2312//IGNORE", '征信查询' . date('Y-m-dHis'));
            //print_r($fileName);exit;
            $fileName = '' . date('Y-m-dHis');//Credit_inquiry
