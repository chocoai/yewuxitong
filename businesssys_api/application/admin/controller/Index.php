...
class Index extends Base {
    ...
    public function fileUpload(){
        ...
        $info = $file->validate(['ext'=> config('uploadFile.file_type')])->move($path,true,false);
        //文件地址
        ...
        if($info){
            ...
            if(in_array($imgInfo['ext'],['png','gif','jpeg','jpg','PNG','GIF','JPEG','JPG'])){
                //缩略图的上传地址
                $date = date('Ymd',time());
            ...
            try{
                ...
                if(in_array($imgInfo['ext'],['png','gif','jpeg','jpg','PNG','GIF','JPEG','JPG'])){
                    return $this->buildSuccess(['id' => $resImg->id,'url' =>config('uploadFile.url').$imgInfo['url'],'name' => $imgInfo['name'],'thumb_url' => config('uploadFile.thum_url').DS.$imgInfo['savename'],'ext' => $imgInfo['ext']]);
                }
                return $this->buildSuccess(['id' => $resImg->id,'url' =>config('uploadFile.url').$imgInfo['url'],'name' => $imgInfo['name'],'ext' => $imgInfo['ext']]);
            }catch (\Exception $e){
    ...
    public function allPropertyNames(){
        ...
        try{
            ...
            $returnInfo['estateOwnerStr'] = implode('、', Db::name('customer')->where(['order_sn' => $orderSn, 'is_seller' => 2, 'status' => 1])->column('cname')); //业主姓名
