...
class Login extends Base {
    ...
    public function index() {
        ...
        $userInfo = SystemUser::where(['username' => $username, 'password' => $password])->find();
        if (!empty($userInfo)) {
            if ($userInfo['status']) {
                //更新用户数据
        ...
        $access = [];
        $system_name = \app\model\System::where(['hide' => 1])->order('sort')->column('id,tagname');
        $userInfo['is_privileged_people'] = false; //是否有特权人权限
        if ($isSupper) {
            ...
            $groups = SystemAuthGroupAccess::where(['uid' => $userInfo['id']])->find();
            if (!empty($groups)) {
                $userInfo['group'] = str2arr($groups->groupid);
                if (in_array('117', $userInfo['group'])) {//判断是否有特权人权限，bs_system_auth_group表id为177的数据
                    $userInfo['is_privileged_people'] = true;
                }
                $access = SystemAuthRule::whereIn('groupid', $userInfo['group'])->group('menu_id')->select();
                $access = Tools::buildArrFromObj($access);
                foreach ($access as $key => $value) {
                    $naccess[$value['system_id']][] = $value;
                }
...
//                    $return['access'][$system_name[$k]] = array_values(array_unique(array_filter(array_column($v, 'url'))));
        ...
        $return['userinfo'] = ['name' => $userInfo['name'], 'username' => $userInfo['username'], 'avatar' => $userInfo['avatar'], 'is_privileged_people' => $userInfo['is_privileged_people']];
