...
class Nuclearcard extends Base {
    ...
    public function nuclearcardList() {
        ...
        $keywords && $where['x.order_sn|e.estate_name|x.bankcard|e.estate_owner'] = ['like', "%{$keywords}%"];
        $where['x.status'] = 1;
        $field = "x.id,x.order_sn,x.bankaccount,x.bankcard,x.openbank,x.verify_card_status,x.check_time,x.verify_card_status,x.account_status,o.type,o.financing_manager_id,e.estate_name,e.estate_owner,y.verify_card_name";
        $creditList = $this->orderguaranteebank->alias('x')
                        ...
                        ->join('__ORDER_VERIFY_CARD__ y', 'y.order_guarantee_bank_id=x.id', 'left')
    ...
    public function nuclearEntryinfo() {
        ...
        if ($id) {
            ...
            //首期款信息
            $resInfo = OrderComponents::orderDp($data['order_sn'], 'dp_strike_price,dp_earnest_money,dp_supervise_guarantee,dp_supervise_buyer,dp_buy_way,dp_now_mortgage');
            if (isset($resInfo)) {
            ...
            //回款方式信息
            $returnInfo['returnMoney'] = OrderComponents::orderReturnMoney($data['order_sn']);
            //监管信息
            $returnInfo['dbBankInfo'] = OrderComponents::orderBankDp($data['order_sn'], 'id,dp_supervise_date,dp_money,dp_organization_type,dp_supervise_bank,dp_supervise_bank_branch,dp_organization');
            //垫资费计算
