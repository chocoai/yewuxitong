...
class OrderWarrant extends Base {
    ...
    public function index() {
        ...
        if ($result === false)
            return $this->buildFailed(ReturnCode::DB_READ_ERROR, '列表读取失败!');
        if ($result['data']) {
    ...
    public function details() {
        ...
        $dpInfo = OrderComponents::orderDp($orderSn, 'dp_strike_price,dp_earnest_money,dp_money,dp_supervise_bank,dp_supervise_bank_branch,dp_buy_way,dp_now_mortgage,dp_redeem_bank');
        if ($dpInfo) {
    ...
    public function updateOwnership() {
        ...
        if (empty($receiptNo) || $receiptImg <= 0)
            return $this->buildFailed(ReturnCode::PARAM_INVALID, '回执编号或者回执未上传!');
    ...
    public function dataSendList() {
        ...
        $deliveryStatus = $this->request->post('deliveryStatus/d', '');
        $fund_channel_id = $this->request->post('fund_channel_id', '');
        ...
        if ($deliveryStatus === 0 || !empty($deliveryStatus)) {
            $where['z.delivery_status'] = $deliveryStatus;
        }
        $fund_channel_id && $where['z.fund_channel_id'] = $fund_channel_id;
    ...
    public function updateData() {
        ...
        Db::startTrans();
        try {
            $stage = Db::name('order')->lock(true)->where(['status' => 1])->value('stage');
//            if (!$stage) {
//                Db::rollback();
//                return $this->buildFailed(ReturnCode::PARAM_INVALID, '资料入架后的订单才能操作!');
//            }//待资料入架就能资料送审  2018.8.21
     ...
     * @apiParam {int}  id   资金渠道id
     * @apiParam {string}  orderSn
     * @apiParam {string}  trust_contract_num 信托合同号
     * @apiParam {string}  push_order_money 推单金额
     * @apiParam {string}  loan_day 借款天数
     */
    public function reviewData() {
        $id = input('post.id', 0, 'int');
        $orderSn = input('post.orderSn', '');
        $trust_contract_num = input('post.trust_contract_num', '');
        $push_order_money = input('post.push_order_money', '');
        $loan_day = input('post.loan_day', '');
        if ($id <= 0 || $orderSn == '' || $trust_contract_num == '' || $push_order_money == '' || $loan_day == '')
            return $this->buildFailed(ReturnCode::PARAM_INVALID, '参数有误!');
        Db::startTrans();
        try {
            $stage = Db::name('order')->lock(true)->where(['status' => 1])->value('stage');
...
//            }//待资料入架就能资料送审  2018.8.21
            ...
            if ($res && $res['delivery_status'] == 1) {
                ...
                if ($modelFundChannel->where(['status' => 1, 'trust_contract_num' => $trust_contract_num])->count() > 0) {
                    Db::rollback();
                    return $this->buildFailed(ReturnCode::PARAM_INVALID, '该信托合同号已经存在!');
                }
                $result = $modelFundChannel->where(['status' => 1, 'id' => $id, 'delivery_status' => 1])->update(['delivery_status' => 2, 'trust_contract_num' => $trust_contract_num, 'push_order_money' => $push_order_money, 'loan_day' => $loan_day]); //审核通过
                if ($result <= 0) {
                    Db::rollback();
                    return $this->buildFailed(ReturnCode::PARAM_INVALID, '操作失败!');
                }
    ...
    public function changeChannel() {
        ...
        Db::startTrans();
        try {
            $stage = Db::name('order')->lock(true)->where(['status' => 1, 'order_sn' => $orderSn])->value('stage');
//            if (!$stage) {
//                Db::rollback();
//                return $this->buildFailed(ReturnCode::PARAM_INVALID, '资料入架后的订单才能操作!');
//            }//待资料入架就能资料送审  2018.8.21
