...
class Zcdc {
    ...
    const CUSTOMER_GETCUSINFOCERTDIT = 'customer/getCusinfocredit'; //查询征信客户列表
    ...
    public function addBuilding($params) {
        ...
        if (empty($params['building_name']))
            return '楼盘名称不能为空';
        if (empty($params['city_id']) || empty($params['district_id']))
            return '城市或者城区不能为空';
        if (mb_strlen($params['building_name']) > 15 || (!empty($params['building_alias']) && mb_strlen($params['building_alias']) > 15 ))
    ...
    public function addUnit($params) {
        if ($params['unit_name'] == '' || empty($params['building_id']))
            return '缺少参数!';
        if (mb_strlen($params['unit_name']) > 15 || (!empty($params['unit_alias']) && mb_strlen($params['unit_alias']) > 15 ))
            return '栋阁名称或栋阁别名超过15个字符!';
    ...
    public function addHouse($params) {
        if ($params['roomno_name'] == '' || empty($params['unit_id']) || empty($params['floor_id']) || empty($params['building_id']))
            return '缺少参数';
        if (mb_strlen($params['roomno_name']) > 8)
            return '房号名称不得超过8个字符';
     ...
     * @return bool|mixed
     */
    public function cerditcustomerList($params) {
        if (!empty($params['type']) && !empty($params['certtype']) && !empty($params['certcode'])) {
            $result = $this->http_post(self::API_URL_PREFIX . self::CUSTOMER_GETCUSINFOCERTDIT, $this->getData($params));
            if ($result) {
                $json = json_decode($result, true);
                return $json;
            }
    ...
    public function addCustomer($params) {
        if (empty($params['cname']) || mb_strlen($params['cname']) > 50  || empty($params['certcode']) || empty($params['certtype']) || empty($params['certother']) || empty($params['ctype'])) {
            return false;
        }
