...
class ChangeExcel extends \PHPExcel_IOFactory {
    ...
    private static function OutinputHeader($objWriter, $fileName) {
        ...
        $dir = date('Y-m');
        $url = '' . ROOT_PATH . '/' . UPDIR . '/' . $dir . '/';
        if (!is_dir($url)){
            mkdir($url);
        }
        $path = '' . ROOT_PATH . '/' . UPDIR . '/' . $dir . '/' . '' . $fileName;
        $objWriter->save($path);
    ...
    public static function RecursionCreateExecl($head, $data, $fileName,$n) {
        $PHPExecl = new \PHPExcel();
        $objWriter = self::createWriter($PHPExecl, 'Excel2007');
        $PHPExecl->getProperties()->setCreator("Office 2007 XLSX Test Document")
        ...
        self::SummerInsertDateToExecl($sheet, $head, $data, $n);
        $backfile = self::OutinputHeader($objWriter, $fileName);
        return $backfile;
    }
