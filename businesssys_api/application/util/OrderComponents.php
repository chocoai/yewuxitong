...
class OrderComponents {
    ...
    public static function orderJbInfo($order_sn) {
        $orderInfo = Db::name('order')->alias('a')
                ...
                ->where(['a.order_sn' => $order_sn, 'a.delete_time' => NULL])
                ->find();
        ...
        if (!empty($data)) {
            foreach ($data as &$value) {
                ...
                foreach ($bankuse as $k => $v) {
                    $bankuse_text['code'] = $bankuse[$k];
                    $bankuse_text['valname'] = $newStageArr[$v];
                    $bankuse_info[] = $newStageArr[$v];
                    $type[] = $bankuse_text;
                }
                $value['type'] = $type; //银行卡用途
                $type = [];
     ...
     * @throws \think\exception\DbException
     */
    public static function orderDp($orderSn, $field = 'dp_strike_price,dp_earnest_money,dp_buy_way,dp_now_mortgage,dp_redeem_bank,dp_redeem_bank_branch,dp_supervise_guarantee,dp_supervise_buyer') {
     ...
     * @throws \think\exception\DbException
     */
    public static function orderBankDp($orderSn, $field = 'dp_supervise_date,dp_money,dp_organization_type,dp_supervise_bank,dp_supervise_bank_branch,dp_organization') {
        $data = Db::name('order_dp_bank')->field($field)->where(['order_sn' => $orderSn, 'status' => 1])->select();
        if (!empty($data)) {
            foreach ($data as &$value) {
                $value['dp_organization_type_text'] = $value['dp_organization_type'] == 1 ? '银行' : '其他';
            }
        }
        return $data;
    }
     ...
     * @throws \think\exception\DbException
     */
    public static function orderReturnMoney($orderSn, $field = 'return_money_mode,return_money_amount,remark') {
        $data = Db::name('order_guarantee_return')->field($field)->where(['order_sn' => $orderSn, 'status' => 1])->select();
        if (!empty($data)) {
            $newStageArr = dictionary_reset((new Dictionary)->getDictionaryByType('ORDER_REPAY_METHOD'));
            foreach ($data as &$value) {
                $value['return_money_mode_str'] = !empty($value['return_money_mode']) ? $newStageArr[$value['return_money_mode']] : '';
            }
        }
        return $data;
    }
        ...
        if (!empty($dispatchInfo)) {
            foreach ($dispatchInfo as $k => $value) {
                if ($value['mortgage_id']) {
                    ...
                    $mortgateInfo[$k]['accumulation_fund'] = $datainfo['out_account'];
            ...
            foreach ($mortgateInfo as $k => $v) {
                ...
                $mortgateInfo[$k]['accumulation_fund'] = $v['out_account']; //原按揭出账金额
...
//        $res['return_money_mode_str'] = !empty($res['return_money_mode']) ? $newStageArr[$res['return_money_mode']] : '';//2018.9.6去掉回款方式
    ...
    public static function showDebitInfo($outid) {
        $data = Db::name('order_ransom_out')->where('id', $outid)->field('prestore_day,order_sn,account_type,receipt_bank,receipt_bank_account,is_prestore,receipt_bank_card,money,ransom_dispatch_id,item,way,bank,cheque_num,create_time')->find();
     ...
     * @param string $outid 出账表id(对应order_ransom_out表)
     */
    public static function showBankInfo($outid) {
        $data = Db::name('order_ransom_out')->where('id', $outid)->field('out_bank_card,out_bank,out_bank_account,outok_time')->find();
        if (!empty($data)) {
            $data['outok_time'] = date('Y-m-d H:i', $data['outok_time']);
        }
        return $data;
    }
    ...
    public static function nuclearentryinfo($id) {
        $data = Db::name('order_verify_card')->where('order_guarantee_bank_id', $id)->field('limit_public_day,limit_private_day,limit_public_single,limit_private_single,id,order_guarantee_bank_id,card_type,verify_card_time,cyber_bank,telephone_bank,mobile_bank,security_account,credit_card,verify_card_data,verify_card_name,remark')->find();
        if (!empty($data)) {
    ...
    public static function advanceMoney($orderSn) {
        $condition['order_sn'] = $orderSn;
        $condition['status'] = 1;
        return Db::name('order_advance_money')->where($condition)->field('advance_money,advance_day,advance_rate,plus_rate,advance_fee,remark,id')->select();
    }
